...
const RegistrationRequests = () => {
  ...
  const [stats, setStats] = useState({ all: 0, active: 0, pending: 0 });
    ...
    try {
      ...
      if (response.data.success !== false) {
        ...
        setStats({
          ...
          pending: data.filter(r => r.status === 'pending' || r.status === 'pending_payment').length,
        });
      } else {
  ...
  const getStatusBadge = (status) => {
    const badges = {
      ...
      suspended: { icon: XCircle, color: 'text-red-600 bg-red-50 border-red-200', label: 'Suspended' },
      inactive: { icon: XCircle, color: 'text-gray-600 bg-gray-50 border-gray-200', label: 'Inactive' },
    };
  ...
  const getFilteredRegistrations = () => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(r => 
        ...
        r.clinic_email?.toLowerCase().includes(term)
      );
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white border rounded-lg p-4 text-center shadow-sm">
          <p className="text-2xl font-bold text-blue-600">
            {registrations.filter(r => r.subscription_plan === 'premium').length}
          </p>
              ...
              <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4">
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-3 mb-3">
                    <h3 className="text-lg font-semibold text-gray-900">{reg.clinic_name || 'N/A'}</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-gray-900">{reg.owner_name || 'N/A'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Mail className="w-4 h-4 text-gray-400" />
                      <span className="text-gray-600">Email:</span>
                      <span className="font-medium text-gray-900">{reg.owner_email || 'N/A'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Phone className="w-4 h-4 text-gray-400" />
                      <span className="text-gray-600">Phone:</span>
                      <span className="font-medium text-gray-900">{reg.owner_phone || 'N/A'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Building2 className="w-4 h-4 text-gray-400" />
                      <span className="text-gray-600">Clinic:</span>
                      <span className="font-medium text-gray-900">{reg.clinic_email || 'N/A'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <CreditCard className="w-4 h-4 text-gray-400" />
                      <span className="text-gray-600">Plan:</span>
                      <span className="font-medium text-gray-900 capitalize">{reg.subscription_plan || 'N/A'}</span>
                    </div>
                    <div className="flex items-center gap-2">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-semibold">{selectedRegistration.clinic_name || 'N/A'}</p>
                </div>
                <div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  <p><span className="text-gray-500">Name:</span> {selectedRegistration.owner_name || 'N/A'}</p>
                  <p><span className="text-gray-500">Email:</span> {selectedRegistration.owner_email || 'N/A'}</p>
                  <p><span className="text-gray-500">Phone:</span> {selectedRegistration.owner_phone || 'N/A'}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  <p><span className="text-gray-500">Email:</span> {selectedRegistration.clinic_email || 'N/A'}</p>
                  <p><span className="text-gray-500">Phone:</span> {selectedRegistration.clinic_phone || 'N/A'}</p>
                ...
                <>
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      ...
                      <p><span className="text-gray-500">Amount:</span> {selectedRegistration.amount || 'N/A'}</p>
                    </div>
                  </div>
                </>
              )}
