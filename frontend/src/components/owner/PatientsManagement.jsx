...
const paymentMethods = [
  ...
  },
  {
    value: 'cash',
    label: 'Cash',
    description: 'Patient pays with cash',
    icon: BanknotesIcon,
  },
  {
    value: 'telebirr',
    label: 'Telebirr',
    description: 'Pay using Telebirr',
  ...
  */
  ...
  const loadPatients = async (search = '') => {
    ...
    try {
      ...
      // ✅ Use api.get() with the token automatically added
      const response = await api.get('/patients', {
        params: search.trim() ? { search: search.trim() } : {},
      });

      // ✅ api automatically handles the response
  ...
  */
  ...
  const openEditModal = (patient) => {
    ...
    setFormData({
      ...
      cash_amount: patient.payment_amount || '',

      telebirr_phone:
        patient.telebirr_phone || '',

      telebirr_reference:
        patient.telebirr_reference || '',

      cbe_birr_phone:
        patient.cbe_birr_phone || '',

      cbe_birr_reference:
        patient.cbe_birr_reference || '',

      bank_name: patient.bank_name || '',

      bank_reference:
        patient.bank_reference || '',

      /*
       * Do not load/store raw card number/CVV
       * from your database.
       *
       * These are only UI values when entering
       * a new card payment.
       */
      card_holder_name: '',
      card_number: '',
  ...
  */
  ...
  const handlePaymentMethodChange = (method) => {
    setFormData((previous) => ({
      ...
      /*
       * Free automatically becomes zero.
       */
      payment_amount:
        method === 'free'
          ? '0'
          : previous.payment_amount === '0'
          ? ''
          : previous.payment_amount,
    }));
  ...
  */
  ...
  const handleSave = async () => {
    ...
    if (
      ...
      !formData.cbe_birr_phone.trim()
    ) {
      toast.error(
        'CBE Birr phone number is required.'
      );
     ...
     * IMPORTANT:
     *
     * For production, raw card information must
     * be handled by a PCI-compliant payment
     * provider. Do not store card number/CVV
     * in your Laravel database.
     */
    ...
    try {
       ...
       * Payment data.
       *
       * NEVER save raw card number/CVV in your
       * Laravel database in production.
       *
       * For a real card gateway, replace the
       * card fields with a secure payment token.
       */
      ...
      const payload = {
        ...
        telebirr_reference:
          formData.payment_method ===
          'telebirr'
            ? formData.telebirr_reference.trim() ||
              null
            : null,
        ...
        cbe_birr_reference:
          formData.payment_method ===
          'cbe_birr'
            ? formData.cbe_birr_reference.trim() ||
              null
            : null,
         ...
         * Card
         *
         * These should be replaced with a secure
         * gateway token for production.
         */
        ...
        /*
         * ⚠️ SECURITY FIX: Only send last 4 digits
         * DO NOT send full card number or CVV!
         */
        card_last_four:
          formData.payment_method === 'card'
        ...
        /*
         * ⚠️ For production: Send card data to a payment gateway
         * like Chapa, Stripe, etc. and get a token.
         *
         * DO NOT send card_number or card_cvv to your Laravel backend!
         */
      ...
      const patientCode =
        response.data?.data?.patient_code ||
        response.data?.patient?.patient_code ||
        '';
      ...
      /*
       * If your Laravel payment system returns
       * a checkout URL, redirect to it.
       */
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[#5B6B72] text-sm">
            Manage real patient records from your clinic database
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 border-l-4 border-[#1FAE6B]">
          ...
          <div className="text-2xl font-heading font-bold text-[#1FAE6B]">
            {summary.female}
          </div>
        </div>
              ...
              <div>
                ...
                <p className="text-sm text-[#5B6B72] mt-0.5">
                  {editingPatient
                    ...
                    : 'Register a new patient and record payment.'}
                </p>
            ...
            <div className="p-6">
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="p-3 rounded-lg bg-white border border-gray-200">
                            ...
                            <div className="text-xs text-[#5B6B72]">
                              ...
                              For production card
                              payments, use a secure
                              payment gateway/tokenization
                              service. Do not save the
                              card number or CVV in your
                              Laravel database.
              ...
              {!editingPatient && (
                ...
                <div className="mt-5 p-3 bg-[#0EA5A5]/5 border border-[#0EA5A5]/20 rounded-lg">
                  ...
                  <div className="text-sm font-medium text-[#0EA5A5]">
                    Patient ID will be generated automatically
                  </div>

                  <div className="text-xs text-[#5B6B72] mt-1">
                    Example: PAT-0016. The code is
                    created by Laravel after the database
                    record is inserted.
                  </div>
